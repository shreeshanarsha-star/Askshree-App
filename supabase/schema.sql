...
-- Gauri.ai free-use gate (same pattern — 3 free requests per IP, lifted for
-- logged-in accounts). Gauri.ai transcribes speech (live recording via
...
create table if not exists gauri_usage (
  ip_address text primary key,
  use_count integer not null default 0,
  status text not null default 'free',
  last_used_at timestamptz default now()
);
alter table gauri_usage enable row level security;
