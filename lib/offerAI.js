...
// Classifies a batch of uploaded documents into the six types Offer.ai cares
// about, from filename + a text snippet of each. This is what powers the
// single "drag everything in, AI sorts it" dropzone — the recruiter never
// tells us which file is which.
const CLASSIFY_PROMPT = `You are sorting a recruiter's uploaded documents for a compensation-offer
tool. You are given each file's name and the first part of its extracted text. Classify EVERY file
into exactly one of: "cv", "appointment_letter", "payslip", "education", "jd", "budget", "other".
- cv: the candidate's own resume/CV
- appointment_letter: a previous employer's appointment/offer letter for this candidate
- payslip: a salary slip / payslip showing pay components
- education: a degree certificate / marksheet
- jd: a job description for the role being hired for
- budget: a budget approval note/sheet for the role
- other: anything that doesn't clearly fit
Respond as JSON only (no markdown fences, no prose): an array, one object per file, in the same
order given: [{ "fileName": string, "docType": string }]`;

export async function classifyOfferDocuments(files) {
  // files: [{ fileName, text }]
  const listing = files
    .map((f, i) => `${i + 1}. ${f.fileName}\n---\n${(f.text || '(no extractable text)').slice(0, 1500)}`)
    .join('\n\n');
  const raw = await askClaude(CLASSIFY_PROMPT, listing, 800);
  const match = raw.match(/\[[\s\S]*\]/);
  const parsed = JSON.parse((match ? match[0] : raw).replace(/```json|```/g, '').trim());
  // Defensive: map back by position in case the model drops a fileName.
  return files.map((f, i) => ({
    fileName: f.fileName,
    docType: (parsed[i] && parsed[i].docType) || 'other',
  }));
}
