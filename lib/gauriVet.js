...
// without a human vet signing off first. No product catalog is wired in
// yet (phase 2), so recommendations stay general rather than naming SKUs.
...
export async function draftTriage({ issueText, cowDetails }) {
  const context = `Cow details: ${cowDetails || 'not provided'}

Farmer's description of the issue:
${issueText}`;

  const raw = await askClaude(TRIAGE_PROMPT, context, 800);
  const parsed = JSON.parse(raw.replace(/```json|```/g, '').trim());
  return parsed;
}

export function formatDraftForVet(draft) {
  const lines = [];
