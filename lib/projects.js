...
export async function updateProjectCandidate({ projectId, candidateId, fields }) {
  const db = supabaseAdmin();
  const allowed = ['status', 'comments', 'qualification', 'current_ctc', 'expected_ctc', 'notice_period', 'outreach_status'];
  const update = {};
...
export async function getProjectCandidates(projectId) {
  ...
  const { data } = await db
    .from('project_candidates')
    .select('id, name, designation, company, location, match_score, profile_url, source, added_at, qualification, current_ctc, expected_ctc, notice_period, status, comments, outreach_status, match_reason')
