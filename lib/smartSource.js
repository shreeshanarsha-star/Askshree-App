...
// --- SignalHire enrichment (Phase 2) ---------------------------------------
// After scoring, the top candidates get a real profile pull from SignalHire's
// Person API using `withoutContacts: true` -- public profile data (skills,
// experience, education) without spending a standard contact credit. This
// uses a separate "without contacts" credit type that SignalHire enables on
// request (contact support@signalhire.com) -- if it's not enabled yet, or
// no SIGNALHIRE_API_KEY is set, this silently no-ops and search results are
// unaffected, same graceful-degradation pattern as contact reveal.
//
// Not persisted to the DB yet (smart_source_candidates has a fixed column
// set) -- this enriches the in-memory response for the current search only.
// Real caching lands as its own phase.
const SIGNALHIRE_SEARCH_URL = 'https://www.signalhire.com/api/v1/candidate/search';
const ENRICH_TOP_N = 30;

export async function enrichTopCandidates(scored) {
  const signalHireKey = process.env.SIGNALHIRE_API_KEY;
  if (!signalHireKey || !scored.length) return new Map();
  ...
  try {
    const res = await fetch(SIGNALHIRE_SEARCH_URL, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', apikey: signalHireKey },
      body: JSON.stringify({
        items: top.map((c) => c.profile_url),
        withoutContacts: true,
        withoutWaterfall: true,
      }),
    });
    if (!res.ok) return new Map(); // 402 (credits not enabled/exhausted) etc -- no-op
    ...
    for (const r of results) {
      ...
      const c = r.candidate;
      // SignalHire returns the literal string "n/a" (not null) for several
      // company fields when data isn't available -- filter those out so we
      // don't render a "n/a employees" chip.
      const clean = (v) => (v && v !== 'n/a' ? v : null);
      const currentRole = c.experience?.find((e) => e.current);
      byUrl.set(r.item, {
        verified_skills: c.skills || [],
        experience_years: c.experienceYears ?? null,
        headline: c.headLine || null,
        current_company_industry: clean(currentRole?.industry),
        current_company_size: clean(currentRole?.companySize),
        education_summary: (c.education || []).map((e) => e.university).filter(Boolean).join(', ') || null,
      });
    }
    return byUrl;
  } catch (e) {
    return new Map(); // network/parse failure -- no-op, search still returns
  }
}
