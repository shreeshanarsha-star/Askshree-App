...
export function buildSearchQuery({ skills, location, roleTitle, booleanQuery }) {
  // Serper's free tier rejects ANY advanced query syntax — not just site:,
  // but quoted exact-phrase terms too ("Query pattern not allowed for free
  // accounts"). Free tier only accepts plain, unquoted keyword strings.
  // Workaround: bias toward LinkedIn with a plain "linkedin" keyword and
  // plain unquoted terms, then filter the response to linkedin.com/in links
  // server-side (already done in the route/search caller).
  if (booleanQuery && booleanQuery.trim()) {
    return `linkedin ${booleanQuery.trim().replace(/["']/g, '')}${location ? ` ${location}` : ''}`;
  }
...
export async function searchSerper(query) {
  ...
  return { ok: true, results: organic, rawCount: (data.organic || []).length, rawLinks: (data.organic || []).map((r) => r.link) };
}
