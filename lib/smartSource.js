...
export function buildSearchQuery({ skills, location, roleTitle, booleanQuery }) {
  // Serper's free tier rejects the site: operator ("Query pattern not
  // allowed for free accounts"). Workaround: bias toward LinkedIn with a
  // plain "linkedin" keyword instead, and filter the response to
  // linkedin.com/in links server-side (already done in the route/search
  // caller) rather than restricting the query itself.
  if (booleanQuery && booleanQuery.trim()) {
    return `linkedin ${booleanQuery.trim()}${location ? ` "${location}"` : ''}`;
  }
