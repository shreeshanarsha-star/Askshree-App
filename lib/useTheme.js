...
export function useTheme() {
  const [themeId, setThemeId] = useState(() => readCache(STORAGE_KEY) || readCache(SITE_DEFAULT_CACHE_KEY) || DEFAULT_THEME);
  const [ready, setReady] = useState(() => !!(readCache(STORAGE_KEY) || readCache(SITE_DEFAULT_CACHE_KEY)));
  const [hasOverride, setHasOverride] = useState(() => !!readCache(STORAGE_KEY));
