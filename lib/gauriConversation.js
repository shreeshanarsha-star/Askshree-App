...
function buildSystemPrompt(turnCount, language) {
...
Respond as JSON only, no markdown fences: { "reply": string, "ready": boolean,
"surfaceDiagnosis": string|null, "suggestedProduct": string|null, "urgency": "routine"|"prompt"|"emergency"|null }
Write the "reply" field in ${language || 'English'} -- the farmer is speaking that language, so reply in it
naturally (not a literal translation, just speak the way a native speaker would). Keep
"surfaceDiagnosis", "suggestedProduct" and "urgency" in English regardless, since a vet reads those.
...
export async function continueConversation({ transcript, cowDetails, language }) {
  ...
  const raw = await askClaude(systemPrompt, context, 700);
  ...
  let suggestedProductRow = null;
  if (parsed.ready && parsed.suggestedProduct) {
    suggestedProductRow = products.find(
      (p) => p.name.toLowerCase() === parsed.suggestedProduct.toLowerCase()
    ) || null;
  }
