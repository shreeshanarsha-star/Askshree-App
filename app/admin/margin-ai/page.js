...
export default function MarginAI() {
  ...
  async function load() {
    const res = await fetch('/api/admin/margin/dashboard', { headers: { Authorization: `Bearer ${token}` } });
  ...
  async function runUpload() {
    ...
    const res = await fetch('/api/admin/margin/upload', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
      body: JSON.stringify({ salesCsv, costCsv, sourceLabel }),
    });
  ...
  return (
    <div className="admin-shell">
      ...
      <div className="admin-main">
                ...
                <tbody>
                  {data.leaks.map((p) => (
                    <tr key={p.id}>
                      ...
                      <td><a href={`/admin/margin-ai/product/${p.id}`} className="row-action">View</a></td>
                    </tr>
                  ))}
        ...
        <div className="panel">
          <div className="panel-head"><h3>Upload sales &amp; cost data</h3><span className="action" onClick={() => window.location.href = '/admin/margin-ai/actions'}>View actions →</span></div>
