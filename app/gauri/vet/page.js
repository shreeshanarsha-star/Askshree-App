...
// needs_callback and show the farmer's phone prominently, since those
// farmers were explicitly told "a vet will call you" — this is not a
// silent-review queue for those, it's a callback worklist. Approving can
...
export default function GauriVetDashboard() {
  ...
  function draftFor(c) {
    ...
    try { return JSON.parse(c.ai_draft); } catch (e) { return null; }
  }
              ...
              <tbody>
                {cases.map((c) => (
                  <tr key={c.id}>
                    ...
                    <td>{c.cow_details || '—'}</td>
                    <td style={{ maxWidth: 260 }}>{c.issue_text?.slice(0, 80)}{c.issue_text?.length > 80 ? '…' : ''}</td>
            ...
            <div style={{ padding: '18px 20px' }}>
              ...
              {draftFor(selected) ? (
                <div style={{ border: '1px solid var(--line)', borderRadius: 8, padding: 14, marginBottom: 16, background: 'rgba(255,255,255,0.015)' }}>
                  ...
                  {draftFor(selected).immediate_care?.length > 0 && (
