...
// payment yet (per product decision): the paramed collects cash/UPI in
// person and just logs a note when marking delivered.
    ...
    <div style={{ border: '1px solid var(--line)', borderRadius: 8, padding: 14, marginBottom: 10, background: 'rgba(255,255,255,0.015)' }}>
      ...
      <div style={{ marginTop: 10, display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
        ...
        {mineList && o.status === 'out_for_delivery' && (
          <>
            <input className="free-text-input" style={{ padding: 8, width: 220 }} placeholder="Payment note e.g. Collected ₹250 cash"
              value={paymentNotes[o.id] || ''} onChange={(e) => setPaymentNotes({ ...paymentNotes, [o.id]: e.target.value })} />
            <button className="primary-btn" style={{ marginTop: 0 }} disabled={busyId === o.id}
              onClick={() => act(o.id, 'mark_delivered', { paymentNote: paymentNotes[o.id] || '' })}>Mark delivered</button>
          </>
        )}
      </div>
    </div>
